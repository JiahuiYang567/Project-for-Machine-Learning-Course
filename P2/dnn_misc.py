...
class linear_layer:
    ...
    def forward(self, X):
        ...
        ################################################################################
        forward_output=np.dot(X,self.params['W'])# N*Dout
        forward_output=forward_output+self.params['b']
    ...
    def backward(self, X, grad):
        ...
        """
            ...
            - grad: A N-by-output_D numpy array, where each 'row' (say row i) is the partial derivatives of the mini-batch loss
                 w.r.t. forward_output[i].l/u (u is N*Dout) (b=1*Dout)
        ...
        ##########################################################################################################################
        #l/W(Din*Dout)=(l/u)(N*Dout)*(u/W)(N*Din)=XT(Din*N)*grad(N*Dout)
        N,D=X.shape
        One=np.ones((1,N))
        self.gradient['W']=np.dot(np.transpose(X),grad)
        self.gradient['b']=np.dot(One,grad)
...
class relu:
    ...
    def forward(self, X):
        ...
        """
            ...
            Input:
            - X: A numpy array of arbitrary shape. (always u(N*Dout))
    ...
    def backward(self, X, grad):
        ...
        """
            ...
            - grad: A numpy array of the same shape of X, where each element is the partial derivative of the mini-batch loss
                 w.r.t. the corresponding element in forward_output. (l/h)
                 )
        ...
        """
        #l/u(N*Dout)=l/h(N*Dout)*h/u(N*Dout)=grad()*H(u)
        ...
        ##########################################################################################################################
        H=self.forward(X)
        self.mask=np.where(H>0)
        H[self.mask]=1
        #print('grad:',grad,"H(u)",H)
        backward_output=grad*H
        return backward_output
...
class dropout:
    ...
    def backward(self, X, grad):
        ...
        """
            ...
            - X: A numpy array of arbitrary shape, the input to the forward pass.
            - grad: A numpy array of the same shape of X, where each element is the partial derivative of the mini-batch loss
                 w.r.t. the corresponding element in forward_output.l/d
            ...
            Return:
            - backward_output: A numpy array of the same shape as X, where each element is the partial derivative of the mini-batch loss
                 w.r.t. the corresponding element in X. l/X=l/d()*d/X
        """
