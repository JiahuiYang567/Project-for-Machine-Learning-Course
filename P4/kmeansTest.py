...
def transform_image(image, code_vectors):
    ...
    W,L,Channel=image.shape
    X=image.reshape(W*L,Channel)
    n_cluster=len(code_vectors)
    Dist=np.zeros((W*L,n_cluster))
    for k in range(n_cluster):
        Dist[:,k]=np.linalg.norm(X-code_vectors[k],axis=1)
    R_image=np.argmin(Dist,axis=1)
    new_im=code_vectors[R_image]
    new_im=new_im.reshape(W,L,Channel)
    #print (new_im.shape)
