...
def pca(X = np.array([]), no_dims = 50):
    ...
    eigen=np.dot(np.transpose(X),X)
    eigen_value, eigen_vector=np.linalg.eig(eigen)
    eigen_value_max=eigen_value.argsort()
    eigen_value_max=eigen_value_max[::-1]
    eigen_vector=eigen_vector[:,eigen_value_max]
    M=eigen_vector[:,0:no_dims]
    Y=np.dot(X,M)
...
def reconstruction_error(orig = np.array([]), decompressed = np.array([])):
    ...
    """TODO: write your code here"""
    D=len(orig)
    meansquare=np.power((orig-decompressed),2)
    error=np.mean(meansquare)
