...
def compute_distances(Xtrain, X):
	...
	for y in range(l2):
		for x in range(l1):
			L2=np.linalg.norm(X[y]-Xtrain[x])
			d.append(L2)
		D.append(d)
		d=[]
	dists=np.array(D)
...
def predict_labels(k, ytrain, dists):
	...
	ypred=[]
	for i in range(dis):
		near=np.argsort(dists[i]) #near is an array that put the index of dists in sorted order
		for j in range(k):
			k_dist.append(ytrain[near[j]])
		label=Counter(k_dist)
		label_1=label.most_common(1)
		y=[k for k,v in label.items() if v==label_1[0][1]]
		y=sorted(y)
		ypred.append(y[0])
		k_dist=[]
